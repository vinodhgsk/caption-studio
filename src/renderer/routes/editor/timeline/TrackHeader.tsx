...
export function TrackHeader({ track, heightPx }: TrackHeaderProps): JSX.Element {
  ...
  return (
    <div
      className="group relative flex items-center justify-center gap-2 border-b border-line px-1 text-text-secondary bg-surface-2/60 hover:bg-surface-2/90 transition-all select-none"
      style={{ height: `${heightPx}px` }}
    >
      {/* Left side: Icon */}
      <span className="text-text-muted shrink-0 mr-1">
        <TrackIcon type={track.type} />
      </span>

      {/* Actions (Lock + Hide/Mute) */}
      ...
      <button
        ...
        className={`flex h-5 w-5 items-center justify-center rounded text-xs transition-colors ${
          locked ? 'text-accent bg-accent/15' : 'text-text-muted hover:text-text-secondary'
        }`}
      ...
      {isAudio ? (
        <button
          ...
          className={`flex h-5 w-5 items-center justify-center rounded text-xs transition-colors ${
            muted ? 'text-danger bg-danger/15' : 'text-text-muted hover:text-text-secondary'
          }`}
        ...
        <button
          ...
          className={`flex h-5 w-5 items-center justify-center rounded text-xs transition-colors ${
            hidden ? 'text-accent bg-accent/15' : 'text-text-muted hover:text-text-secondary'
          }`}
      ...
      <button
        ...
        className="absolute right-0 top-0 bottom-0 opacity-0 group-hover:opacity-100 w-6 flex items-center justify-center bg-danger/90 text-white transition-opacity z-10"
      >
